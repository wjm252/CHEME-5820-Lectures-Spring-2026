...
\begin{tikzpicture}[
    ...
    conn/.style  = {line width=1.0pt, draw=black!50},
    connarr/.style = {line width=1.0pt, draw=black!50,
                    -{Stealth[length=6pt, width=5pt]}},
    resconn/.style = {line width=1.0pt, draw=black!40, rounded corners=6pt},
    resconnarr/.style = {line width=1.0pt, draw=black!40, rounded corners=6pt,
                    -{Stealth[length=6pt, width=5pt]}},
...
\def\yQKV{4.8}       % raised to give Stealth arrows room above LN1
\def\yAttn{7.0}
\def\yAddA{8.8}      % first residual add
\def\yLNB{10.4}      % LayerNorm before FFN
\def\yFF{12.2}
\def\yAddB{14.0}     % second residual add
\def\yOut{15.8}
\def\resoff{5.8}
...
\node[smallblock, fill=blue!8, draw=blue!40] (wq) at (-3.2, \yQKV)
    {\textbf{Q}\\[1pt]$\tilde{\mathbf{X}}\mathbf{W}_{Q}$};
\node[smallblock, fill=blue!8, draw=blue!40] (wk) at (0, \yQKV)
    {\textbf{K}\\[1pt]$\tilde{\mathbf{X}}\mathbf{W}_{K}$};
\node[smallblock, fill=blue!8, draw=blue!40] (wv) at (3.2, \yQKV)
    {\textbf{V}\\[1pt]$\tilde{\mathbf{X}}\mathbf{W}_{V}$};
...
%% ---- Input -> LN1 ----
\draw[connarr] (input.north) -- (ln1.south);

%% ---- Fan-out: LN1 -> Q, K, V ----
\coordinate (branch) at (0, {0.5*(\yLNA + \yQKV) + 0.15});
\draw[conn] (ln1.north) -- (branch);
\fill[black!45] (branch) circle (2.5pt);
\draw[conn] (-3.2, {0.5*(\yLNA + \yQKV) + 0.15})
         -- ( 3.2, {0.5*(\yLNA + \yQKV) + 0.15});
\draw[connarr] (-3.2, {0.5*(\yLNA + \yQKV) + 0.15}) -- (wq.south);
\draw[connarr] (0,    {0.5*(\yLNA + \yQKV) + 0.15}) -- (wk.south);
\draw[connarr] ( 3.2, {0.5*(\yLNA + \yQKV) + 0.15}) -- (wv.south);

%% ---- Q, K, V -> Attention ----
\draw[connarr] (wq.north) -- ([xshift=-3.2cm]attn.south);
\draw[connarr] (wk.north) -- (attn.south);
\draw[connarr] (wv.north) -- ([xshift=3.2cm]attn.south);

%% ---- Attention -> Add1 ----
\draw[connarr] (attn.north) -- (add1.south);

%% ---- Add1 -> LN2 -> FF -> Add2 -> Output ----
\draw[connarr] (add1.north) -- (ln2.south);
\draw[connarr] (ln2.north)  -- (ff.south);
\draw[connarr] (ff.north)   -- (add2.south);
\draw[connarr] (add2.north) -- (output.south);

%% ---- Residual 1: Input bypasses LN1+Attention -> Add1 ----
\draw[resconnarr] (input.east) -- (\resoff, \yInput)
    -- (\resoff, \yAddA) -- (add1.east);
\node[lbl, font=\scriptsize, text=black!35]
    at (\resoff + 0.55, {0.5*(\yInput + \yAddA)})
    {\rotatebox{90}{residual}};

%% ---- Residual 2: Add1 output bypasses LN2+FFN -> Add2 ----
\draw[resconnarr] ([yshift=0.1cm]add1.east) -- (\resoff, {\yAddA + 0.1})
