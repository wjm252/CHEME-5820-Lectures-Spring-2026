...
%% MAIN-PATH NODES
%% ============================================================
\node[inblock]  (u)  at (\xU,  \yMain) {\textbf{Input}\\[1pt]$\mathbf{u}_{t}\in\mathbb{R}^{d_{\text{in}}}$};
\node[matB]     (B)  at (\xB,  \yMain) {$\bar{\mathbf{B}}$\\[1pt]\footnotesize $(h\,d_{\text{in}})\times d_{\text{in}}$};
\node[plus]     (s1) at (\xSa, \yMain) {$+$};
\node[opblock]  (x)  at (\xX,  \yMain) {\textbf{State}\\[1pt]$\mathbf{x}_{t}\in\mathbb{R}^{h\,d_{\text{in}}}$};
\node[matC]     (C)  at (\xC,  \yMain) {$\bar{\mathbf{C}}$\\[1pt]\footnotesize $d_{\text{out}}\times (h\,d_{\text{in}})$};
\node[plus]     (s2) at (\xSb, \yMain) {$+$};
\node[outblock] (y)  at (\xY,  \yMain) {\textbf{Output}\\[1pt]$\mathbf{y}_{t}\in\mathbb{R}^{d_{\text{out}}}$};

%% skip row (feedforward)
\node[matD]     (D)  at (\xMid, \ySkip)
    {$\bar{\mathbf{D}}$\\[1pt]\footnotesize $d_{\text{out}}\times d_{\text{in}}$};

%% feedback row
\node[delay]    (z)  at (\xX,  \yFb) {$z^{-1}$\\[1pt]\footnotesize delay};
\node[matA]     (A)  at (\xSa, \yFb) {$\bar{\mathbf{A}}$\\[1pt]\footnotesize block-diag};
...
\node[titlelbl, anchor=south] at (\xMid, \yTitle)
    {Discrete-time MIMO structured state space model};

%% ============================================================
%% EQUATION CAPTION
%% ============================================================
\node[eqnlbl, anchor=north] at (\xMid, \yCaption)
    {state: $\mathbf{x}_{t} = \bar{\mathbf{A}}\,\mathbf{x}_{t-1} + \bar{\mathbf{B}}\,\mathbf{u}_{t}$
     \qquad\qquad
     output: $\mathbf{y}_{t} = \bar{\mathbf{C}}\,\mathbf{x}_{t} + \bar{\mathbf{D}}\,\mathbf{u}_{t}$};
